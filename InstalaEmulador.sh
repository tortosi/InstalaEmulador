...
conecta="-h ${host} --port ${port} -u ${user} -p ${pass}"
...
####################################################################
		...
		"1 - Configurar authserver.conf y worldserver.conf" "" \
		"2 - Configurar tabla realmlist de la base de datos auth" "" \
		"0 - Volver" "" 2> var55
...
####################################################################
		if [ "$opcion55" = "1 - Configurar authserver.conf y worldserver.conf" ]; then
			dialog --title "Menú de opciones --- Creado por MSANCHO" \
			--backtitle $backtitle \
			--nocancel \
...
####################################################################
			if [ "$opcion56" = "1 - Configurar archivo authserver.conf" ]; then
				...
				if [ ! -x $server_tricore/$conf7  ];then
				mkdir $server_tricore/$conf
				fi

				sed -e "s/sqluser/$user/g" -i $server_tricore/etc/authserver.temp

				sed -e "s/sqlpass/$pass/g" -i $server_tricore/etc/authserver.temp

				sed -e "s/dbauth/$auth/g" -i $server_tricore/etc/authserver.temp
...
####################################################################
				...
				if [ ! -x $server_tricore/$conf7  ];then
				mkdir $server_tricore/$conf7
				fi

				sed -e "s/sqluser/$user/g" -i $server_tricore/etc/world.conf

				sed -e "s/sqlpass/$pass/g" -i $server_tricore/etc/world.conf

				sed -e "s/dbauth/$auth/g" -i $server_tricore/etc/world.conf

				sed -e "s/dbworld/$world/g" -i $server_tricore/etc/world.conf

				sed -e "s/dbchar/$char/g" -i $server_tricore/etc/world.conf
...
####################################################################
		elif [ "$opcion55" = "2 - Configurar tabla realmlist de la base de datos auth" ]; then
